...
#        tonnetz = librosa.feature.tonnetz(y=y, sr=sr)
        ...
        tempo_vector_norm.set_value(id, tempo/120.0) 
        ...
        timbre_var.set_value(id, 0.2*np.mean(zcr) + 0.4*np.mean(cent) + 0.3*np.mean(rolloff) + 0.1*np.mean(spec_bw))
    ...
    feature_set['intensity'] = rmse_mean  # root mean squared energy
    feature_set['timbre'] = timbre_var  # root mean squared energy
    feature_set['rhythm'] = rhythm_var  # root mean squared energy
 ...
 #   feature_set['rmse_var'] = rmse_var
    ...
    feature_set['cent_mean'] = cent_mean  # centroid
